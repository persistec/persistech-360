...
const controlBaseClass = [
  'flex w-full rounded-md border border-border bg-surface px-3 py-2 text-sm text-foreground shadow-sm shadow-black/5 transition-colors',
...
export function Label({ className = '', children, ...props }: LabelProps) {
  return (
    <label
      className={`mb-2 block text-sm font-medium leading-none text-foreground ${className}`}
      {...props}
    >
      {children}
    </label>
...
export function FormField({
  ...
  return (
    <div className={className}>
      ...
      {error ? (
        <p id={errorId} className="mt-1 text-xs leading-5 text-danger">
          {error}
        </p>
      ) : null}
    </div>
